...
class MainViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        items = [
            Subject(title: "Custom slider", description: ""),
            Subject(title: "Increment fixed value", description: "")
        ]
    ...
    func showCustomSlider() {
        if let vc = CustomSliderController.fromStoryboard(Storyboards.CustomSlider.name) as? CustomSliderController {
            self.navigationController?.pushViewController(vc, animated: true)
        }
    }
    
    func showIncrementFixedValue() {
        if let vc = IncrementFixedValueController.fromStoryboard(Storyboards.IncrementFixedValue.name) as? IncrementFixedValueController {
            self.navigationController?.pushViewController(vc, animated: true)
        }
    }
}
