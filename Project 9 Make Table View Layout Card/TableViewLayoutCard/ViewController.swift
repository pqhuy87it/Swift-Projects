...
class ViewController: UIViewController {
	...
	override func viewDidLoad() {
		...
		// setup tableview
		tableView.backgroundColor = UIColor.color(245, green: 245, blue: 245)
		let insets = UIEdgeInsets(top: 0, left: 0, bottom: (tabBarController?.tabBar.bounds.size.height ?? 0) + 12 * 2, right: 0)
		tableView.contentInset = insets
		tableView.scrollIndicatorInsets = insets
		tableView.registerCellByNib(CustomTableViewCell.self)
	}
...
extension ViewController: UITableViewDataSource {
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		if let cell = tableView.dequeueCell(CustomTableViewCell.self, forIndexPath: indexPath) {
			return cell
		}
